...
<h4>12. React Component Lifecycle : the "life cycle" refers to the series of events or phases that a component goes through during its existence, from its creation to its removal from the DOM. In React class components have different methods that get called at specific points during their life cycle, allowing developers to perform actions or handle behavior at different stages of a component's existence. But when Functional Components took over, then all those methods have got replaced by hook called "useEffect", it tracks 3 basic things of a component : Mount(Birth/First Render) , Update (State Update mostly), Unmount(Death/Removal from DOM).</h4>